...
const ModalEdit = ({isOpen, setIsOpen, setEditingJob, editingJob}) => {
  ...
  return (
    ...
    <div>
        <form onSubmit={(e)=>updateJob(e, editingJob.id)}  className="relative border border-slate-200 bg-slate-200 rounded-xl px-4 py-6 flex flex-col gap-4">
            ...
            <button
              type="submit"
              className="px-4 cursor-pointer py-2 bg-sky-600 text-slate-100 font-medium rounded-md hover:opacity-90"
            >
              {isOpen ? "Update" : "Add"}
            </button>
